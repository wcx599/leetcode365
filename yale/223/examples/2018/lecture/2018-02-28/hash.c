...
hashFunction(const char *key)
{
    ...
    for(size_t i = 0; key[i] != '\0'; i++) {
        // adding more operators does not
        // necessarily make the hash function
        // better, but it's fun
        h = h << 5 + h * 97 ^ (3*key[i]);
    }
...
static void
hashPrintHelper(const char *key, int value, void *data)
{
    printf("%s %d\n", key, value);
}
...
hashPrint(const Hash *h) {
    hashTraverse(h, hashPrintHelper, 0);
}

void 
hashTraverse(const Hash *h, void (*f)(const char *key, int value, void *), void *data) {
    for(size_t i = 0; i < h->size; i++) {
        if(h->a[i].key != 0) {
            f(h->a[i].key, h->a[i].value, data);
        }
    }
}
