...
/* hash function won't look at anything past this length */
#define HASH_PREFIX_LENGTH (32)
...
struct dict {
    ...
    /* this is the array of hash values */
    unsigned long hashArray[HASH_PREFIX_LENGTH][UCHAR_MAX+1];
};
...
internalDictCreate(int size)
{
    ...
    for(i = 0; i < HASH_PREFIX_LENGTH; i++) {
        for(j = 0; j < UCHAR_MAX+1; j++) {
            d->hashArray[i][j] = rand();
        }
    }
...
DictSearch(Dict d, const char *key)
{
    ...
    for(e = d->table[hash_function(d, key) % d->size]; e != 0; e = e->next) {
        if(!strcmp(e->key, key)) {
            /* got it */
            return e->value;
        }
...
DictDelete(Dict d, const char *key)
{
    ...
    for(prev = &(d->table[hash_function(d, key) % d->size]); 
        *prev != 0; 
        prev = &((*prev)->next)) {
